...
- When a user loads the home page, they should be redirected to a page that lists all of the users' names. Load the users from the users.yaml file (content below). DONE
- Each of the users' names should be a link to a page for that user. DONE
- On each user's page, display their email address. Also, display their interests, with a comma appearing between each interest. DONE
- At the bottom of each user's page, list links to all of the other users pages. Do not include the user whose page it is in this list. DONE
